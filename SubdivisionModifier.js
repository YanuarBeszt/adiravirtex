...
THREE.SubdivisionModifier.prototype.smooth = function (oldGeometry) {
  ...
  function assert() {
    if (scope.debug && console && console.assert)
  ...
  var sharpEdges = {},
    sharpVertices = []; // Mark edges and vertices to prevent smoothening on them
  ...
  function getUV(vertexNo, oldFaceNo) {
    ...
    if (!theUV) {
      if (
        vertexNo >= originalVerticesLength &&
        vertexNo < originalVerticesLength + originalFaces.length
      ) {
        debug("face pt");
      } else {
        debug("edge pt");
  ...
  debug(
    "originalFaces, uvs, originalVerticesLength",
